...
public class LeetCode_416_PartitionEqualSubsetSum_BottomUp_SpaceOptimized {
     ...
     * 
     * From #4. It is clear that only previous state is required and not whole table
     * i.e only one row is sufficient.
     * 
    ...
    public boolean canPartition(int[] nums) {
        ...
        for (int idx = n - 1; idx >= 0; idx--) {
            for (int currentSum = 0; currentSum <= target; currentSum++) {
                boolean skip = memo[currentSum];
                boolean take = currentSum + nums[idx] <= target ? memo[currentSum + nums[idx]] : false;
                memo[currentSum] = take || skip;
            }
        }
        return memo[0];
    }
